...
@api_router.post("/developer/seed-test-data")
async def seed_test_data(user=Depends(require_role("developer"))):
    """Seed complete test data for all roles. Idempotent — checks before creating."""
    from uuid import uuid4
    ...
    created = {
        ...
        "accountant": None,
        "payment_config": None,
        "fee_ledger": {"kitty": 0, "meeting_fee": 0},
        "meetings": 0,
        "attendance": 0,
        "legacy_bridge": 0,
    }
    ...
    try:
        ...
        created["superadmin"] = f"error: {str(e)}"

    # --- 2. Subscription (Free Trial, 30 days, 1 chapter) ---
    try:
        ...
        created["subscription"] = f"error: {str(e)}"
    ...
    try:
        ...
        if existing_chapter:
            ...
            await db.chapters.insert_one({
                "chapter_id": chapter_id,
                "name": "BNI Sunrise Test Chapter",
                "created_by": ed_mobile,
        ...
        created["chapter"] = f"error: {str(e)}"

    # --- 4. Members (5 test members) ---
    member_specs = [
        ...
        try:
            ...
            if existing_member:
                ...
                member_data = {
                    "member_id": member_id,
                    "chapter_id": chapter_id,
                    "unique_member_id": spec["uid"],
                    "full_name": spec["name"],
                    "primary_mobile": spec["mobile"],
                    "secondary_mobile": None,
                    "email": None,
                    "business_name": spec["biz"],
                    "business_category": spec["cat"],
                    "joining_date": None,
                    "renewal_date": None,
                    "induction_fee": None,
                    "membership_status": "active",
                    "status": "Active",
                    "created_at": now_iso,
                    "bni_member_id": spec["uid"],
                    "organization_id": chapter_id,
                    "status_history": [{
                        "action": "created",
                        "from_status": None,
                        "to_status": "active",
                        "reason": "Seeded test member",
            ...
            # Member credentials (idempotent via mobile unique index)
            ...
            created["members"].append({"mobile": spec["mobile"], "status": f"error: {str(e)}"})
    ...
    try:
        ...
        if existing_acct:
            ...
            await db.accountant_credentials.insert_one({
                "accountant_id": str(uuid4()),
                "superadmin_id": superadmin_id,
                "name": "Vikram Accountant",
                "mobile": acct_mobile,
        ...
        created["accountant"] = f"error: {str(e)}"
    ...
    try:
        ...
        if existing_pc:
            ...
            await db.payment_config.insert_one({
                "superadmin_id": ed_mobile,
                "upi_id": "testED@upi",
                "upi_holder_name": "Test ED",
                "upi_qr_data": None,
                "bank_enabled": True,
                "bank_account_name": "Test ED",
        ...
        created["payment_config"] = f"error: {str(e)}"

    # --- 7. Fee Ledger (Current Month) ---
    try:
        ...
        # Kitty statuses by member index: M1=verified, M2=verified, M3=submitted, M4=pending, M5=pending
        ...
        for idx, member in enumerate(member_records):
            ...
            for fee_type, amount in [("kitty", 2000.0), ("meeting_fee", 500.0)]:
                ...
                if fee_type == "kitty":
                    status = kitty_statuses[idx]
                else:
                    status = "pending"

                # Build timeline progressively
                timeline = [{"action": "created", "by": "developer", "role": "system",
                             "at": now_iso, "note": "Seeded test fee"}]
                ...
                if status in ("submitted", "admin_confirmed", "verified"):
                    timeline.append({"action": "submitted", "by": member.get("primary_mobile", ""),
                                     "role": "member", "at": now_iso, "note": "Test payment proof"})
                    payment_method = "upi"
                    utr_number = f"UTR{m_id[:8].upper()}"
                    payment_date = now_iso

                if status in ("admin_confirmed", "verified"):
                    timeline.append({"action": "admin_confirmed", "by": admin_mobile,
                                     "role": "admin", "at": now_iso, "note": "Admin confirmed"})

                if status == "verified":
                    timeline.append({"action": "verified", "by": ed_mobile,
                                     "role": "superadmin", "at": now_iso, "note": "ED verified"})
        ...
        created["fee_ledger"]["error"] = str(e)

    # --- 8. Legacy Bridge (kitty_payments for verified M1, M2) ---
    try:
        for idx in [0, 1]:
            ...
            if not existing_legacy:
                await db.kitty_payments.insert_one({
                    "payment_id": f"KP{now.strftime('%Y%m%d%H%M%S')}{idx}",
                    "chapter_id": chapter_id,
                    "member_id": m_id,
                    "member_name": member.get("full_name", ""),
                    "month": current_month,
        ...
        created["legacy_bridge"] = f"error: {str(e)}"

    # --- 9. Meetings (last 4 Wednesdays) ---
    wednesdays = []
    meeting_ids = []
    try:
        d = now.date()
        ...
        if days_since_wed == 0:
            days_since_wed = 7  # Use last Wednesday, not today
        last_wed = d - timedelta(days=days_since_wed)
        for i in range(4):
            wednesdays.append(last_wed - timedelta(weeks=i))
        wednesdays.reverse()  # oldest first

        for wed_idx, wed_date in enumerate(wednesdays):
            ...
            late_cutoff_utc = start_utc + timedelta(minutes=30)
            ...
            await db.meetings.insert_one({
                "meeting_id": mt_id,
                "chapter_id": chapter_id,
                "date": wed_date.isoformat(),
                "start_time": start_utc.isoformat(),
                "late_cutoff_time": late_cutoff_utc.isoformat(),
        ...
        created["meetings"] = f"error: {str(e)}"

    # --- 10. Attendance ---
    try:
        # attendance_matrix[member_idx][week_idx] = present?
        attendance_matrix = [
            [True,  True,  True,  True],   # M1: all 4
            [False, True,  True,  True],   # M2: absent week 1 (oldest)
            [False, False, True,  True],   # M3: absent weeks 1,2
            [False, False, False, True],   # M4: absent weeks 1,2,3
            [True,  True,  True,  True],   # M5: all 4
        ]
        ...
        created["attendance"] = f"error: {str(e)}"
    ...
    return {
        ...
        "login_credentials": {
            "developer": {
                "note": "Use your existing developer account",
                "endpoint": "/api/developer/login",
                "email": "admin@aasaanapp.com",
                "password": "AasaanAdmin2026!"
            },
            "superadmin_ed": {
            ...
            "members": [
                {"endpoint": "/api/member/login", "mobile": spec["mobile"],
                 "password": "Test@1234", "role": "member", "name": spec["name"]}
                for spec in member_specs
            ],
            "accountant": {
                "endpoint": "/api/accountant/login",
                "mobile": "9999900008",
                "password": "Test@1234",
                "role": "accountant",
                "name": "Vikram Accountant"
        ...
        "chapter_id": chapter_id,
        "test_data_summary": {
            "ed": "9999900001 / Test@1234",
            "admin": "9999900002 / Test@1234",
            "members": "9999900003-07 / Test@1234",
            "accountant": "9999900008 / Test@1234",
            "chapter": "BNI Sunrise Test Chapter",
            "fees": f"Kitty ₹2000 + Meeting ₹500 for {month_name} {current_year}",
            "meetings": f"4 meetings (last 4 Wednesdays)",
            "attendance": "M1,M5=4/4, M2=3/4, M3=2/4, M4=1/4"
        }
    }
