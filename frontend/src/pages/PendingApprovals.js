  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200 px-8 py-4">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-600">Substitute & Visitor attendance requests</p>
          </div>
          <Button
      ...
      <div className="p-8 max-w-7xl mx-auto">
        {loading ? (
          <div className="text-center py-12">Loading pending requests...</div>
        ) : pending.length === 0 ? (
          <Card className="p-12 text-center">
            <Clock className="h-16 w-16 text-slate-300 mx-auto mb-4" />
            <p className="text-slate-600">No pending approval requests</p>
            <p className="text-sm text-slate-500 mt-2">Substitute and visitor attendance will appear here</p>
          </Card>
        ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2">
                      {att.type === 'substitute' && (
                        <>
                          <div>
                            <p className="text-sm text-slate-500">Substitute For:</p>
                            <p className="font-semibold text-slate-900">
                              Member ID: {att.unique_member_id} ({att.member_name})
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-slate-500">Substitute Details:</p>
                            <p className="font-medium text-slate-900">{att.substitute_name}</p>
                            <p className="text-sm text-slate-600">{att.substitute_mobile}</p>
                          </div>
                        </>
                      )}
                      ...
                      {att.type === 'visitor' && (
                        <>
                          <div>
                            <p className="text-sm text-slate-500">Visitor Details:</p>
                            <p className="font-medium text-slate-900">{att.visitor_name}</p>
                            <p className="text-sm text-slate-600">{att.visitor_mobile}</p>
                            {att.visitor_company && (
                              <p className="text-sm text-slate-600">Company: {att.visitor_company}</p>
                            )}
                          </div>
                          <div>
